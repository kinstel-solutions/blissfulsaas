...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.patientAppUrl = this.configService.get<string>('PATIENT_APP_URL') || 'http://localhost:3000';
    this.therapistAppUrl = this.configService.get<string>('THERAPIST_APP_URL') || 'http://localhost:3001';
    this.adminPanelUrl = this.configService.get<string>('ADMIN_PANEL_URL') || 'http://localhost:3002';
  ...
  private generateHtmlWrap(title: string, bodyText: string, buttonText?: string, buttonUrl?: string): string {
  ...
  async sendTherapistApplicationReceived(to: string, therapistName: string) {
    ...
    return this.sendEmail(to, subject, this.generateHtmlWrap(title, body, 'View Portal Status', `${this.therapistAppUrl}/auth/login`));
  }
  ...
  async sendAdminNewApplication(adminEmails: string[], therapistName: string) {
    ...
    return this.sendEmail(adminEmails.join(','), subject, this.generateHtmlWrap(title, body, 'Review Application', `${this.adminPanelUrl}/therapists`));
  }
  ...
  async sendTherapistWelcomeEmail(to: string, therapistName: string) {
    ...
    return this.sendEmail(to, subject, this.generateHtmlWrap(title, body, 'Set Up Your Profile', `${this.therapistAppUrl}/dashboard`));
  }
  ...
  async sendTherapistRejectionEmail(to: string, therapistName: string, reason: string) {
    ...
    return this.sendEmail(to, subject, this.generateHtmlWrap(title, body, 'Contact Support', 'mailto:support@theblissfulstation.com'));
  }
  ...
  async sendTherapistProfileUpdatesApproved(to: string, therapistName: string) {
    ...
    return this.sendEmail(to, subject, this.generateHtmlWrap(title, body, 'View Your Profile', `${this.therapistAppUrl}/profile`));
  }
  ...
  async sendTherapistProfileUpdatesRejected(to: string, therapistName: string) {
    ...
    return this.sendEmail(to, subject, this.generateHtmlWrap(title, body, 'View Profile Settings', `${this.therapistAppUrl}/profile`));
  }

  // Appointment Lifecycle
  async sendAppointmentNotification(to: string, title: string, body: string, buttonText?: string, buttonUrl?: string) {
    return this.sendEmail(to, title, this.generateHtmlWrap(title, body, buttonText, buttonUrl));
  }
}
