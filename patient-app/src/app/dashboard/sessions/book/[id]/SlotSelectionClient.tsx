...
import { Clock, Calendar, ArrowRight, CheckCircle2, AlertCircle, CreditCard } from "lucide-react";
...
export default function SlotSelectionClient({ therapist, slots }: { therapist: any; slots: any[] }) {
  ...
  const [orderData, setOrderData] = useState<any>(null); // populated → shows modal
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      // If mock mode → show our modal. If live → launch Razorpay SDK (future)
      setOrderData(order);
    } catch (err: any) {
  ...
  return (
    <>
      {/* Mock Payment Modal (shown when order is created) */}
      {orderData && (
        <MockPaymentModal
          orderData={orderData}
          onClose={() => {
                        ...
                        {daySlots.map((slot) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {selectedSlot?.id === slot.id && !booked && (
