...
import {
  ...
  Send, Ban, Paperclip, FileText, X, ExternalLink, Loader2, User,
} from "lucide-react";
...
export default function MessageHistoryClient({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
      setUploadError(`File too large. Maximum is ${MAX_FILE_SIZE_MB} MB.`);
      setTimeout(() => setUploadError(""), 4000);
      return;
    }
          ...
          {!selectedSession ? (
            ...
            <>
              ...
              <div className="p-3 md:p-6 bg-white border-b border-slate-100 flex items-center justify-between sticky top-0 z-10">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-lg md:text-sm font-bold text-slate-900 leading-none truncate">
                      {mode === "therapist"
                        ...
                        : selectedSession.therapist?.firstName}
                    </h3>
                ...
                {mode === "therapist" && selectedSession.patient?.id && (
                  <Link
                    href={`/dashboard/patients/${selectedSession.patient.id}`}
                    className="flex items-center justify-center gap-1.5 px-3 py-2 bg-primary text-white hover:bg-primary/95 hover:shadow-md active:scale-95 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-xl transition-all shadow-sm shrink-0"
